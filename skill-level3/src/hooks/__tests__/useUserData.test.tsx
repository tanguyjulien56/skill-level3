...
describe('UserProfileCard', () => {
  it('should render the user\'s image and name', () => {
    ...
    expect(screen.getByRole('img')).toHaveAttribute('src', '/no_image_available.png'); // Par défaut l'image de remplacement
    expect(screen.getByAltText(`${mockUserData.firstName} ${mockUserData.lastName}`)).toBeInTheDocument();

    // Vérifie que le nom complet est bien affiché
    expect(screen.getByText(`${mockUserData.firstName} ${mockUserData.lastName}`)).toBeInTheDocument();
  });
  ...
  it('should display the correct number of days to birthday', () => {
    ...
    expect(screen.getByText('Votre anniversaire est dans 331 jours')).toBeInTheDocument();
  });
  ...
  it('should display an error message when no birthDate is provided', () => {
    ...
    expect(screen.getByText("Date d'anniversaire invalide")).toBeInTheDocument();
  });
});
